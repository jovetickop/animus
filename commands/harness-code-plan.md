...
2. 用户选择后立即追加写入 `.claude/harness-cc/plan-context.md`（格式参考 `${CLAUDE_PLUGIN_ROOT}/templates/harness/plan-context.md`）
3. 用户选择"其他（自定义）"时等待用户输入文本
4. 全部 7 问完成后，AI 读取完整的 `plan-context.md` 作为后续规划输入

> **术语提取**：Grilling 全部完成后，从用户回答中提取领域术语，写入 `.claude/harness-cc/domain-lexicon.md`。特别关注 Q1（验收标准）中的业务概念和 Q5（架构约束）中的技术术语。
