...
| 标签 | 定义 | 处理方式 |
|------|------|---------|
| **intent_gap** | 意图捕获不完整导致的 bug（不是代码问题） | 需求理解本身不对 → 回滚 → 找用户确认意图 |
| **bad_spec** | spec 边界不够强，实现走偏 | 代码按 spec 写了，但 spec 没覆盖异常 → 回滚 → 修 spec → 重做 |
| **patch** | **本次改动**引入的缺陷 | 问题行在 diff 范围内 → 直接自动修复 |
| **defer** | **改动前已存在**的存量问题 | 问题行不在 diff 范围内 → 记入 deferred-work.md |
...
agent 超时不等于失败。网络延迟或服务端繁忙时自动延长等待时间，继续等待不中断。仅在 agent 明确返回错误时重试（最多 3 次），仍错误则终止报错。
...
辅助验证脚本：
- `python animus-engine.py validate` — 验证 features.json 结构
- `python animus-engine.py status` — 检查状态一致性
