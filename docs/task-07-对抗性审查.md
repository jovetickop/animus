...
| 标签 | 定义 | 处理方式 | 是否阻塞 |
|------|------|---------|---------|
| `intent_gap` | 意图捕获不完整导致的 bug（需求理解本身不对） | 回滚 → 找用户确认意图 | 阻塞 |
| `bad_spec` | spec 边界不够强，实现走偏（不是代码写错） | 回滚 → 修 spec → 重做 | 阻塞 |
| `patch` | 局部代码写错了 | 自动修复 | 不阻塞，自动处理 |
| `defer` | 存量问题，不是本次改动引入的 | 记入 `deferred-work.md`，不打断主线 | 不阻塞 |
| `reject` | 误报 | 静默丢弃 | 不阻塞 |

**处理顺序：** 先处理 intent_gap 和 bad_spec（代码会被重推），再处理 patch。defer 和 reject 不阻塞流程。

**defer 机制说明：**
审查中发现与本次改动无关的存量问题（pre-existing bugs、已有代码坏味道），记录到项目根 `.claude/animus/deferred-work.md`，不混入当前修复流程。下次 `/animus-dev` 启动时自动读取，作为参考但不强制。deferred-work.md 格式：

```markdown
# 延迟工作记录

## 2026-07-04 — 审查 T005 时发现
- [ ] `src/legacy_parser.cpp:120` 缺乏输入长度检查（存量问题）
- [ ] `src/database.cpp:45` 连接池没有超时机制（存量问题）
```
