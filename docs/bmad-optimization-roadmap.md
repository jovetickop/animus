...
**目标：** `/animus-dev` 命令：五路路由（debug/oneshot/fast/light/full）。AI 自动选路 + 用户确认，`autonomous = true` 时跳过确认。
...
| Quick Dev oneshot | 参考 BMAD `bmad-quick-dev` 的 `step-oneshot.md` 路径 |
