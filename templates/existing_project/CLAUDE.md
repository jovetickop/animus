...
```powershell
.\.claude\harness-cc\coding-session.ps1
python .claude/harness-cc/show-status.py
...
- 开始任务：`.\.claude\harness-cc\update-progress.ps1 <TaskId> in_progress "说明" [-AutoPush]`
- 完成任务：`.\.claude\harness-cc\update-progress.ps1 <TaskId> passed "完成说明" [-AutoPush]`
- 任务受阻：`.\.claude\harness-cc\update-progress.ps1 <TaskId> failed "失败原因与下一步" [-AutoPush]`
...
- 每个任务完成并通过验收后，按 `.claude/rules/universal/git-workflow.md` 执行提交。
