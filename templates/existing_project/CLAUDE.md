...
当前项目以 `animus` 的长任务工作流插件模式运行。技能安装目录 `${CLAUDE_PLUGIN_ROOT}`（Claude Code 自动设置）。
...
- 以 `.claude/animus/features.json` 中的 `acceptance_criteria` 和 `test_command` 为准。
- 至少执行构建和测试验证。
- 没有构建/测试证据时，不得宣称任务完成。
...
- 如果当前目录不是 Git 仓库，可跳过提交，但必须在 `.claude/animus/animus-history.jsonl` 记录原因。

## 核心节奏

- 每轮只推进一个任务
- 每个任务结束都要构建和测试
- 每次结果都追加到 `.claude/animus/animus-history.jsonl`
- 每次状态流转后自动更新 `.claude/animus/docs/<任务编号>-任务描述.md`

## 必须长期维护的文件

- `.claude/animus/features.json`
- `.claude/animus/animus-history.jsonl`
- `.claude/animus/feature-detail.md`
- `.claude/animus/domain-lexicon.md`
- `.claude/animus/task_plan.md`
- `.claude/animus/findings.md`
- `.claude/animus/adr/`
- `.claude/animus/docs/`
