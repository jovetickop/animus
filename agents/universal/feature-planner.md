...
- 产出适合写入 `.claude/harness-cc/features.json` 的小粒度任务。
...
1. 读取 `.claude/harness-cc/features.json` 中所有任务的状态
...
- `.claude/harness-cc/plan-context.md`（若存在，Grilling 追问结果，规划时优先参考）
- `.claude/harness-cc/features.json`（若存在）
- 项目根目录 `CLAUDE.md`（若存在）
...
> **feature-detail 同步**: 每次规划完成后，同步写入/更新 `.claude/harness-cc/feature-detail.md`，将已规划和已实现的任务状态标记到对应子功能列表中。
...
如果 `.claude/harness-cc/plan-context.md` 存在，说明已经过 Grilling 追问阶段。规划时应优先参考其中的用户回答，确保任务列表与用户实际需求对齐。plan-context.md 中的 Q1~Q7 回答分别对应验收标准、依赖、异常处理、性能安全、架构约束、风险评估和测试策略，应作为每个任务拆解的核心依据。

### 术语提取

Grilling 结束后，从用户回答中提取领域术语，写入 `.claude/harness-cc/domain-lexicon.md`。
