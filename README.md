...
| 问题 | harness-cc 的解法 |
|------|-------------------|
| **跨会话失忆** | 每次启动先读 `.claude/harness-cc/features.json` + `.claude/harness-cc/harness-history.jsonl` + `.claude/harness-cc/task_plan.md`，恢复现场 |
...
| **会话中断恢复困难** | 会话恢复机制：`/clear` 后自动从 `.claude/harness-cc/` 重建上下文，无需重新初始化 |
| **编码兼容问题** | 自动检测 GBK 编码文件，在读写时透明进行 GBK-UTF-8 双向转换，避免中文乱码 |
| **会话恢复脆弱** | session-catchup.py 直接读文件输出 5 问恢复检查，不再依赖 /clear |
| **任务进度太粗** | task_plan.md 子步骤追踪 + findings.md 知识积累，粒度从任务到步骤 |
| **失败信息缺失** | 维护 `harness-history.jsonl` 记录每次失败上下文，内置 stall 检测识别卡住的任务 |
| **Token 消耗过高** | features.json 按需加载（分片机制），缩减约 78% 的 Token 消耗 |
| **脚本维护困难** | update-progress.ps1 拆分为 5 个领域模块（validate-transition / oracle-runner / report-generator / task-helpers / git-helper），主脚本精简到 59 行 |
| **编码不一致** | 所有 .ps1 文件统一为 UTF-8 with BOM 编码，.sh 文件统一为 LF 换行符，.gitattributes + .editorconfig 提供制度化约束 |
...
flowchart TB
    ...
    subgraph 持久化层[持久化层 · State]
        F[".claude/harness-cc/features.json"]
        J[".claude/harness-cc/harness-history.jsonl"]
        P[".claude/harness-cc/task_plan.md"]
        K[".claude/harness-cc/findings.md"]
    end
    subgraph 运行时引擎[运行时引擎 · Harness]
        H["harness/update-progress.ps1（薄编排器）"]
        M["harness/modules/（5 领域模块）"]
...
```powershell
# 手动安装（备用）
git clone https://github.com/jovetickop/Harness-CC.git <本地路径>
# 然后在 Claude Code 中执行：
# /plugin install <本地路径>
```

安装后，在任意项目目录中使用三个斜杠命令即可。
...
| `/harness-code-plan` | 有新需求/任务需要拆解时 | 先进入 Grilling 追问阶段（7 问），再转为 `features.json` 任务列表。追问结果保存到 plan-context.md |
...
### Step 1: 读取状态
读取 `.claude/harness-cc/features.json` 和 `harness-history.jsonl`，判断当前阶段。
...
### Step 3: 标记开始
```bash
.\.claude\harness-cc\update-progress.ps1 T001 in_progress "开始..."
```
...
### Step 6: 验证
执行构建命令和测试命令。可用 `.\.claude\harness-cc\run-regression.ps1` 一键执行。
...
### Step 8: 完成或失败
```bash
.\.claude\harness-cc\update-progress.ps1 T001 passed "说明"
# 或
.\.claude\harness-cc\update-progress.ps1 T001 failed "失败原因"
```
...
│   ├── cpp-cmake/                        ← 纯 C++/CMake 规范（含 encoding.md）
│   ├── qt/                               ← Qt 专属规范 + UI 架构
│   ├── python/                           ← Python 规范
│   ├── node/                             ← Node.js 规范
│   ├── rust/                             ← Rust 规范
...
│
├── scripts/                              ← 插件自带脚本
│   └── session-catchup.py               ← 会话恢复工具（/clear 后自动重建上下文）
│
├── skills/                               ← 子技能
...
└── templates/                            ← 安装时使用的模板
    ...
    │   │   ├── oracle-runner.ps1         ← Oracle 验证
    │   │   ├── report-generator.ps1      ← 报告生成
    │   │   ├── task-helpers.ps1          ← 任务辅助
    │   │   └── git-helper.ps1            ← Git 操作
...
状态由 `update-progress.ps1` 脚本（及其 ./modules/ 子目录下的 5 个领域模块）管理，每步都做合法性校验：
...
当 `project-config.json` 中配置了 `verify_config.verify_enabled = true` 时，每次任务从 `in_progress` 转为 `passed` 前自动执行 `verify_command`：

- 验证通过（exit 0）→ 允许转为 `passed`
- 验证失败（exit 非 0）→ 自动转为 `failed`
- 验证超时 → 自动转为 `failed`
- 验证输出写入 `harness-history.jsonl`
...
# 在目标项目目录执行
/harness-cc
# 技能自动检测项目类型，从插件目录直接加载 agents/rules/scripts
```

首次使用时，技能自动检测项目类型（CMakeLists.txt、Cargo.toml、go.mod、package.json、pyproject.toml），从插件目录（通过 `${CLAUDE_PLUGIN_ROOT}` 解析）加载对应的 agents、rules、hooks，并在目标项目创建 `.claude/harness-cc/` 运行时目录和 `features.json` 初始任务列表。
...
```bash
.\.claude\harness-cc\show-status.ps1
# 输出示例:
...
```bash
# Step 1: 标记开始
.\.claude\harness-cc\update-progress.ps1 T003 in_progress "开始实现登录模块"

# Step 2: 阅读对应语言的 architect agent 指导
# Step 3: 实现代码
# Step 4: 代码审查
# Step 5: 构建验证
.\.claude\harness-cc\run-regression.ps1

# Step 6: 验收审查
/harness-code-review

# Step 7: 标记完成（自动触发 Oracle 验证）
.\.claude\harness-cc\update-progress.ps1 T003 passed "登录模块实现完成"
```
...
// .claude/harness-cc/project-config.json
{
  ...
  "verify_config": {
    "verify_enabled": true,
    "verify_command": ".\\.claude\\harness-cc\\run-regression.ps1",
...
如果会话中断（/clear、关机、断网均可）：
```bash
python scripts/session-catchup.py --project-dir .
# 自动读取 .claude/harness-cc/ 下的状态文件，输出 5 问恢复检查
# 【问题 1/5】当前任务  【2/5】完成进度  【3/5】最近失败
# 【问题 4/5】上下文变更  【5/5】下一步建议
# 然后继续工作流
```
...
.\.claude\harness-cc\validate-features.ps1
# 输出:
# PASSED: 6 个任务，结构验证通过
# 或
...
| **PreCompact** | 上下文压缩前 | 向 `harness-history.jsonl` 追加 compact 事件 |
...
`.mcp.json` 作为模板随插件分发，安装时由 `init-project.ps1` 复制到目标项目根目录。如不需要，可删除。
...
| 脚本 | 用途 |
|------|------|
| `templates/harness/update-progress.ps1` | 状态机主入口（薄编排器，逻辑在 5 个模块中） |
| `templates/harness/modules/*.ps1` | 5 个领域模块（validate-transition / oracle-runner / report-generator / task-helpers / git-helper） |
| `templates/harness/run-regression.ps1` | 一键构建+测试运行器 |
| `templates/harness/show-status.py` | 状态显示（增强版：持续时间+预测+失败趋势） |
| `scripts/session-catchup.py` | 会话恢复（5 问重启检查，不依赖 /clear） |
| `hooks/scripts/format-all.py` | 多语言格式化分发（black/prettier/cargo fmt/clang-format） |
| `hooks/scripts/encoding-bridge.py` | GBK/UTF-8 编码桥接 |
| `commands/*.ps1` | 3 个斜杠命令的后端实现 |
...
| 类别 | 数量 |
|------|------|
| Agent 定义 | 22（2 base/ 核心 + 5 universal + 1 frontend + 4 qt + 1 cpp-cmake + 2 python + 3 node + 2 rust + 2 go） |
| 规则文件 | 12（3 universal + 2 cpp-cmake + 2 qt + 1 python + 1 node + 1 rust + 1 go + 1 frontend） |
| 斜杠命令 | 3（harness-code-setup / harness-code-plan / harness-code-review） |
| 技能 | 2（harness-cc + tdd-workflow） |
| 状态机模块 | 5（validate-transition / oracle-runner / report-generator / task-helpers / git-helper） |
| 语言插件 | 6 种（cpp-qt / cpp-cmake / python / node / rust / go） |
| Hooks 脚本 | 10（pre-tool-use / pre-compact / stop-check / clang-format 双版本 + format-all.py + encoding-bridge.py） |
| 基础设施文档 | 3（ENCODING.md / TEMPLATES.md / HOOK-DEBUGGING.md，含 5 问重启检查） |
