...
| 问题 | harness-cc 的解法 |
|------|-------------------|
| **跨会话失忆** | 每次启动先读 `.claude/harness-cc/features.json` + `.claude/harness-cc/claude-progress.txt`，恢复现场 |
...
flowchart TB
    ...
    subgraph 持久化层[持久化层 · State]
        ...
        A[".claude/harness-cc/features.archive.json"]
        L[".claude/harness-cc/claude-progress.txt"]
    end
...
└── templates/                            ← 安装时使用的模板
    ...
    │   └── README.md                     ← 运行时说明
    ├── state/                            ← 初始状态文件模板
    │   ├── features.json                 ← 初始任务清单模板
    │   └── claude-progress.txt           ← 进度日志（空占位）
...
| `scripts/session-catchup.py` | 会话恢复（/clear 后自动重建上下文） |
...
| 基础设施文档 | 3（ENCODING.md / TEMPLATES.md / HOOK-DEBUGGING.md） |
