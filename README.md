...
`harness-cc` 是一个 Claude Code 插件，面向**需要多轮编码会话的复杂任务**。它不是一个普通模板，而是一个**编码工作流引擎**——把 PRD/方案自动拆为可执行任务，按状态机逐个推进，验收后提交。支持 C++/Qt、Python、Node.js、Rust、Go。
...
Claude Code 做长周期开发有几个硬伤，harness-cc 的解法：

| 问题 | 解法 |
|------|------|
| **跨会话失忆** | 每次启动读 `features.json` + `harness-history.jsonl` + `task_plan.md`，恢复现场 |
...
| 步骤 | 动作 |
|------|------|
| 1 | 读取 `features.json` + `harness-history.jsonl`，判断当前阶段 |
...
├─ 编排层 ────────────────────────────────┤
│  /harness-code-setup  /harness-code-plan │
│  /harness-code-review                    │
├─ 执行层 ────────────────────────────────┤
│  agents/{universal,qt,python,node,rust}  │
│  rules/{universal,qt,python,node,rust}   │
├─ 持久化层 ──────────────────────────────┤
│  features.json  harness-history.jsonl    │
...
├── commands/{harness-code-setup,plan,review}
