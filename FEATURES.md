...
## 七、新建工程模板（`new_project`）

位于 `.claude/templates/new_project/`：

| 类别 | 文件 | 说明 |
|------|------|------|
| **构建入口** | `CMakeLists.txt` | Qt5/Qt6 双支持、MSVC/MinGW、VLD、CTest 集成、DLL 自动拷贝 |
| **构建预设** | `CMakePresets.json` | VS 2022 x64，base/debug/release |
| **本地预设** | `CMakeUserPresets.json.template` | 本地 Qt 路径覆写 |
| **主入口** | `src/main.cpp.template` | QApplication 启动 |
| **主窗口** | `src/mainwindow.h/.cpp.template` | Ui::MainWindow 模式 |
| **UI 布局** | `ui/mainwindow.ui.template` | 主窗口 UI 文件 |
| **UI 布局** | `ui/dialog_base.ui.template` | 对话框 UI 文件 |
| **资源** | `resources/resources.qrc.template` | Qt 资源文件 |
| **烟雾测试** | `tests/test_smoke.cpp.template` | QTest 主窗口可见性测试 + CTest 注册 |
| **VLD 查找** | `cmake/FindVLD.cmake` | Visual Leak Detector 自动发现 |
| **第三方库** | `3rd_party/CMakeLists.txt` | 可选扩展点 |
| **第三方库** | `3rd_party/SomeLib/` | 示例三方库骨架 |
| **PRD 模板** | `docs/00-prd.md` | 需求文档模板 |
| **任务拆分** | `docs/01-task-breakdown.md` | 任务拆分表模板 |
| **进度报告** | `docs/progress/T000-template.md` | 单任务执行总结模板 |
| **目录占位** | `bin/`、`build/`、`include/` | `.gitkeep` 保持目录结构 |
